...
N = 4
ind = np.arange(N) + bar_width/2 # The x locations for the groups
...
ax1.set_ylim([0, 5])
ax1.set_xlim([0, ind[3] + bar_width + bar_width + bar_width/2])

# Labelling
ax1.text(ind[0]  - 0.09, df['y1_label'][0]+0.1,
         str(df['y1_label'][0]), color='black', fontweight='bold')
ax1.text(ind[1]  - 0.09, df['y1_label'][1]+0.1,
         str(df['y1_label'][1]), color='black', fontweight='bold')
ax1.text(ind[2]  - 0.09, df['y1_label'][2]+0.1,
         str(df['y1_label'][2]), color='black', fontweight='bold')
ax1.text(ind[3]  - 0.09, df['y1_label'][3]+0.1,
         str(df['y1_label'][3]), color='black', fontweight='bold')

ax1.text(ind[0] + bar_width  - 0.09, df['y2_label'][0]+0.1,
         str(df['y2_label'][0]), color='black', fontweight='bold')
ax1.text(ind[1] + bar_width  - 0.09, df['y2_label'][1]+0.1,
         str(df['y2_label'][1]), color='black', fontweight='bold')
ax1.text(ind[2] + bar_width  - 0.09, df['y2_label'][2]+0.1,
         str(df['y2_label'][2]), color='black', fontweight='bold')
ax1.text(ind[3] + bar_width  - 0.09, df['y2_label'][3]+0.1,
         str(df['y2_label'][3]), color='black', fontweight='bold')

# Set the label and legends
ax1.set_ylabel("Runtime (s)", fontsize=32)
ax1.set_xlabel("Total number of servers", fontsize=32)
plt.legend(loc='upper left', fontsize=26)

ax1.tick_params(axis='x', labelsize=32)
ax1.tick_params(axis='y', labelsize=32)

plt.savefig('scalability_#servers_no_medco+.pdf', format='pdf')
