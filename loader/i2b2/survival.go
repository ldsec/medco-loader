...
func SurvivalTablesLoadingScript() (loading string) {
	...
	loading += `CREATE OR REPLACE FUNCTION grouping_filter(patient integer,group1 integer[], group2 integer[],group3 integer[], group4 integer[])  RETURNS int AS \$\$` + "\n"
	loading += `DECLARE` + "\n"
	loading += `	res int:=0;` + "\n"
	loading += `	group_idx int := 0;` + "\n"
	loading += `BEGIN` + "\n"
	loading += `	IF patient = ANY (group1) THEN` + "\n"
	loading += `		RETURN 1;` + "\n"
	loading += `	END IF;` + "\n"
	loading += `    IF patient = ANY (group2) THEN` + "\n"
	loading += `		RETURN 2;` + "\n"
	loading += `	END IF;` + "\n"
	loading += `	IF patient = ANY (group3) THEN` + "\n"
	loading += `		RETURN 3;` + "\n"
	loading += `	END IF;` + "\n"
	loading += `	IF patient = ANY (group4) THEN` + "\n"
	loading += `		RETURN 4;` + "\n"
	loading += `	END IF;` + "\n"
	loading += `	RETURN -1;` + "\n"
	loading += `END;` + "\n"
