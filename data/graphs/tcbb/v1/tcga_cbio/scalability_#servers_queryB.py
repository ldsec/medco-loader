...
raw_data_query = {
    'x_label':  ['3', '6', '9', '10'],
    'query_tagging': [0.304, 0.414, 0.566, 0.603],                # Query tagging
    'shuffling_key_switching': [0.006, 0.015, 0.027, 0.03],       # Shuffling + Key Switching
    'total': [0.31, 0.43, 0.58, 0.63],                         # Total
}
...
# Labelling
ax1.text(ind[0] - 0.11, df['total'][0]+0.015,
         str(df['total'][0]), color='black', fontweight='bold')
ax1.text(ind[1] - 0.11, df['total'][1]+0.015,
         str(df['total'][1]), color='black', fontweight='bold')
ax1.text(ind[2] - 0.11, df['total'][2]+0.025,
         str(df['total'][2]), color='black', fontweight='bold')
ax1.text(ind[3] - 0.11, df['total'][3]+0.015,
...
plt.savefig('./figures/scalability_#servers_queryB.pdf', format='pdf')
